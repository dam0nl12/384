...
BOARDS = [[[3], [11, 21, 3, 0], [12, 22, 2, 1], [13, 23, 33, 6, 3], [31, 32, 5, 0]],
          ...
          [[5], [11, 12, 21, 22, 10, 0], [13, 14, 23, 24, 34, 18, 0], [15, 25, 35, 2, 1], [31, 32, 33, 1, 1],
           [41, 42, 43, 51, 52, 53, 600, 3], [44, 54, 55, 2, 2], [45, 3]],
          [[6], [11, 12, 13, 2, 2], [14, 15, 3, 1], [16, 26, 36, 11, 0], [21, 22, 23, 2, 2], [24, 25, 34, 35, 40, 3],
           [31, 41, 51, 61, 14, 0], [32, 33, 42, 43, 52, 53, 3600, 3], [44, 54, 64, 120, 3], [45, 46, 55, 56, 1, 1],
           [62, 63, 5, 1], [65, 66, 5, 0]]]
...
class TestStringMethods(unittest.TestCase):
    ...
    def helper_bne_grid(self, board):
        ...
        diff_const_count = (board[0][0] + board[0][0]) * board[0][0] * (
                    board[0][0] - 1) // 2  # number of all binary diff constraints
        cons = csp.get_all_cons()
        bin_count = 0  # number of binary constraints
        for c in cons:
            if len(c.get_scope()) == 2:
                bin_count += 1
        self.assertEqual(bin_count, diff_const_count,
                         "Wrong number of binary not equal constraints for binary_ne_grid!")
    ...
    def test_ord_dh_1(self):
        ...
        cons = []
        con1 = Constraint("C1", [a])
        cons.append(con1)
        con2 = Constraint("C2", [a, b])
        cons.append(con2)
        con3 = Constraint("C3", [a, b, c])
        cons.append(con3)
        for c in cons:
            simpleCSP.add_constraint(c)

        var = []
        var = ord_dh(simpleCSP)
        # Answer should be Var-A.
        self.assertEqual(var.name, simpleCSP.vars[0].name, "DH Picked the wrong variable")

        cons2 = []
        con4 = Constraint("C4", [a, e])
        cons2.append(con4)
        con5 = Constraint("C5", [e])
        cons2.append(con5)
        con6 = Constraint("C6", [e])
        cons2.append(con6)
        con7 = Constraint("C7", [e])
        cons2.append(con7)
        con8 = Constraint("C8", [e])
        cons2.append(con8)
        for c in cons2:
            simpleCSP.add_constraint(c)

        var = ord_dh(simpleCSP)
        # Answer should be Var-E.
        self.assertEqual(var.name, simpleCSP.vars[-1].name, "DH Picked the wrong variable")

    # Added case 2.
    @unittest.skipUnless(TEST_HEURISTICS, "Not Testing Heuristics.")
    def test_ord_dh_2(self):
        a = Variable('A', [1])
        b = Variable('B', [1])
        d = Variable('D', [1])
        simpleCSP = CSP("Simple", [a, b, d])

        cons = []
        con1 = Constraint("C1", [a, b])
        cons.append(con1)
        con2 = Constraint("C2", [a, b])
        cons.append(con2)
        con3 = Constraint("C3", [a, d])
        cons.append(con3)
        for c in cons:
            simpleCSP.add_constraint(c)

        cur_var = simpleCSP.get_all_vars()
        cur_var[0].assign(1)

        var = []
        var = ord_dh(simpleCSP)
        # Answer should be Var-B.
        self.assertEqual(var.name, simpleCSP.vars[1].name, "DH Picked the wrong variable")
    ...
    def test_val_lcv_1(self):
        ...
        print(val_lcv(queens, curr_vars[1]))
        print("HAHA")
    ...
    def test_simple_FC(self):
        ...
        for i in range(len(curr_vars)):
            self.assertEqual(var_domain[i], answer[i],
